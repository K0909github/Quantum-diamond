...
BIN_WIDTH = 5  # Å
DISTANCE_DEPTH_MIN = 100.0  # Å  (この深さ以上のNだけで距離を計算)
...
def main() -> None:
    ...
    pts_for_distance = [p for p in pts if (SURFACE_Z - p[2]) >= DISTANCE_DEPTH_MIN]
    if len(pts_for_distance) < 2:
        print(
            f"{path.name}: N={len(depths)} depth>={DISTANCE_DEPTH_MIN:.0f}Å のNが"
            f"{len(pts_for_distance)}個のため、平均距離は計算しません"
        )
    else:
        mean_distance = mean_pair_distance(pts_for_distance)
        print(
            f"{path.name}: N={len(depths)} (depth>={DISTANCE_DEPTH_MIN:.0f}Å: {len(pts_for_distance)}) "
            f"mean_distance={mean_distance:.3f} Å"
        )
