...
def _extract_xyz_from_tokens(tokens: list[str]) -> tuple[float, float, float] | None:
	# Common OVITO export (space-separated) example:
	# ParticleType  x  y  z  occupancy
	# 1 -4.45875 -20.5102 74.0153 0
	if len(tokens) < 3:
		return None

	# If first token is an integer and next three are floats, treat them as x,y,z.
	try:
		int(tokens[0])
		if len(tokens) >= 4:
	...
	# Fallback: use last 3 numeric columns.
	last3 = tokens[-3:]
...
def main() -> None:
	...
	parser.add_argument(
		...
		default=None,
		help=(
			"OVITO から出力した vacancy 座標ファイル (xyz/csv/空白区切りを想定)。"
			"省略時はカレントディレクトリの vacancy_list を使用します。"
		),
	)
	parser.add_argument(
		"--bin-width",
	...
	vacancy_path = args.vacancy_file or (Path.cwd() / "vacancy_list")
	if not vacancy_path.exists():
		print("vacancy 座標ファイルが見つかりません。")
		print(f"指定: {vacancy_path}")
		print("引数で vacancy ファイルを指定するか、カレントディレクトリに vacancy_list を置いてください。")
		return

	coords = read_vacancy_coordinates(vacancy_path)
	if not coords:
		print("vacancy 座標を読み取れませんでした。ファイル形式（xyz/csv）を確認してください。")
		return

	z_values = [z for _, _, z in coords]
	out_path = args.out or (vacancy_path.parent / "vacancy_distribution_z.png")
